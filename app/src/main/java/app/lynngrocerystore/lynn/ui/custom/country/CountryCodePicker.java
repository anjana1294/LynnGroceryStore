...
public class CountryCodePicker extends RelativeLayout {
    ...
    // These name should match value of enum <attr name="language" format="enum"> from attrs
    ...
    TextInputEditText editText_selectedCountry;
...
//        Log.d(TAG, "Initialization of CCP");
        ...
        editText_selectedCountry = (TextInputEditText) holderView.findViewById(R.id.et_selected_country);
        holder = (RelativeLayout) holderView.findViewById(R.id.layout_country_holder);
        imageViewFlag = (ImageView) holderView.findViewById(R.id.iv_country_flag);
        linearFlagHolder = (LinearLayout) holderView.findViewById(R.id.layout_flag_holder);
        codePicker = this;
        applyCustomProperty(attrs);
        editText_selectedCountry.setOnClickListener(countryCodeHolderClickListener);
    }
...
//        Log.d(TAG, "Applying custom property");
        ...
        try {
            ...
            if (isInEditMode()) {
                ...
                contentColor = a.getColor(R.styleable.CountryCodePicker_contentColor, context.getResources().getColor(R.color.colorBlack));
            }
            if (contentColor != 0) {
            ...
            editText_selectedCountry.setText(e.getMessage());
        } finally {
            a.recycle();
        }
...
//        Log.d(TAG, "Setting default country:" + defaultCountry.logString());
    ...
    void setSelectedCountry(Country selectedCountry) {
        ...
        if (!hideNameCode) {
            if (showFullName) {
                editText_selectedCountry.setText(selectedCountry.getName().toUpperCase() + "  +" + selectedCountry.getPhoneCode());
            } else {
                editText_selectedCountry.setText("(" + selectedCountry.getNameCode().toUpperCase() + ")  +" + selectedCountry.getPhoneCode());
            }
        } else {
            editText_selectedCountry.setText("+" + selectedCountry.getPhoneCode());
        }
...
//            Log.d(TAG, "No country for nameCode " + defaultCountryNameCode + " is found");
    ...
    public void setContentColor(int contentColor) {
        ...
        editText_selectedCountry.setTextColor(this.contentColor);
    }
    ...
    public void setCcpClickable(boolean ccpClickable) {
        ...
        if (!ccpClickable) {
            editText_selectedCountry.setOnClickListener(null);
            editText_selectedCountry.setClickable(false);
            editText_selectedCountry.setEnabled(false);
        } else {
            editText_selectedCountry.setOnClickListener(countryCodeHolderClickListener);
            editText_selectedCountry.setClickable(true);
            editText_selectedCountry.setEnabled(true);
        }
    }
